    ...
    for (index, investment) in enumerate(investments):
        ...
        try:
            with django.db.transaction.commit_on_success():
                dirty_dates.update(_populate_prices(investment, fix_existing))
                dirty_dates.update(_populate_dividends(investment, fix_existing))
                dirty_dates.update(_populate_splits(investment, fix_existing))
